...
txtBusca.addEventListener('input', function() {
    ...
    const btnReset = document.getElementById('btnReset');
        ...
        btnReset.onclick = function() {
            if (confirm("Isso apagará toda a contagem atual. Tem certeza?")) {
                localStorage.removeItem('estoque_salvo'); // Deleta o que foi salvo no "cérebro" do browser
                produtos = []; // Limpa a lista na memória
                document.getElementById('lista').innerHTML = ''; // Limpa a tela
                if(document.getElementById('csvInput')) document.getElementById('csvInput').value = ''; // Reseta o seletor de arquivo
                alert("Sistema resetado. Pronto para nova importação.");
        }
    };
}
